...
# Config is read from environment variables when set, so keys/URLs never have
# to live in this file (and never leak into git). run_all.sh wires these up.
import os as _os

# Your bot's URL (where your bot is running)
BOT_URL = _os.environ.get("BOT_URL", "http://127.0.0.1:8081")

# Choose your LLM provider: "openai", "anthropic", "gemini", "deepseek", "groq", "ollama", "openrouter"
LLM_PROVIDER = _os.environ.get("LLM_PROVIDER", "openrouter")

# Your API key (paste your key here)
LLM_API_KEY = _os.environ.get("LLM_API_KEY", "")  # <-- PUT YOUR API KEY HERE
LLM_MODEL = _os.environ.get("LLM_MODEL", "")  # <-- Optional: specify model or leave empty for default

# For Ollama only: local server URL
OLLAMA_URL = "http://localhost:11434"

# Which test to run by default (override: pass as argv[1], e.g. "full_evaluation")
...
class GroqProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        last_err = None
        for attempt in range(4):                      # Groq free TPM: retry on 429
            req = urlrequest.Request(
                "https://api.groq.com/openai/v1/chat/completions",
                data=json.dumps({"model": self.model, "messages": messages,
                                "temperature": 0.2, "max_tokens": 1500}).encode("utf-8"),
                headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json",
                         "User-Agent": "sutra-judge/1.0"}  # Groq CDN 403s Python-urllib default UA
            )
            try:
                resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
                data = json.loads(resp.read().decode("utf-8"))
...
class OpenRouterProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        import time as _time
        last_err = None
        for attempt in range(6):                      # free pools flap: retry on 429/503
            req = urlrequest.Request(
...
def main():
    ...
    # Run the judge (argv[1] overrides TEST_SCENARIO, e.g. "full_evaluation")
